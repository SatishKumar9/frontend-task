...
const Card = props => {
    const { data, cardSelected, driveItems, setDriveItems, removeChild, duplicateChild } = props;
    let cardImage;
    let backgroundColor;
    ...
    return (
        <div className={`card-container ${isDirectory ? "pointer" : ""}`} onClick={() => { if (isDirectory) { cardSelected({ crumb: data, type: "card" }) } }}>
            ...
            <div className="folder-card-content" style={{ borderTop: "1px solid rgba(0, 0, 0, 0.125)" }}>
                <div>
                    {(!isDirectory) ? <span className="card-ext">{_.split(data.name, ".")[1]}<br /></span> : ""}
                    <span>{(isDirectory) ? data.name : _.split(data.name, ".")[0]}</span>
                </div>
                <Dropdown onClick={e => e.stopPropagation()} align="end" className="options-align">
                    <Dropdown.Toggle variant="success">
                        <img src={dots} alt="dot" />
                    </Dropdown.Toggle>
                    <Dropdown.Menu>
                        <Dropdown.Item eventKey="1" onClick={() => { setActionType("rename"); setShow(true) }}>Rename {(isDirectory) ? "folder" : "file"} </Dropdown.Item>
                        <Dropdown.Item eventKey="2" onClick={() => { duplicateChild(data) }}>Duplicate {(isDirectory) ? "folder" : "file"}</Dropdown.Item>
                        <Dropdown.Item eventKey="3" onClick={() => { setActionType("delete"); setShow(true) }}>Delete {(isDirectory) ? "folder" : "file"}</Dropdown.Item>
                    </Dropdown.Menu>
                </Dropdown>
                {show && <Modal onClose={() => setShow(false)} type={(isDirectory) ? "folder" : "file"} actionType={actionType} data={data} renameItem={renameItem} deleteItem={deleteItem}/> }
            </div>
        </div>
    )
}
